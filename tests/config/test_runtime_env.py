...
""",
    ...
    assert context.env.require(INMET_API_KEY_ENV) == "workspace-key"
...
""",
    ...
    assert context.env.require(INMET_API_KEY_ENV) == "process-key"
    assert context.env.require(INMET_API_KEY_ENV, explicit="explicit-key") == "explicit-key"
...
def test_inmet_api_key_missing_raises(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match=INMET_API_KEY_ENV):
        context.env.require(INMET_API_KEY_ENV)
